    ...
    def _import(self, folder):
        ...
        for name in os.listdir(folder):
            ...
            for row in csv:
                ...
                if row["Typ operacji"] == cleanText("Dywidendy") and row["Opis"].startswith(
                    cleanText("DVCA Dywidenda pieniężna")
                ):
                    ...
                    if "rozliczenie" in row["Opis"]:
                        ...
                        sub = Action(time, EActionType.INCOME, income, self.currency(row["Waluta"]))
                    ...
                    if "podatek" in row["Opis"] and prev_dividend:
                        sub = Action(time, EActionType.TAX, income, self.currency(row["Waluta"]))
                        prev_dividend.addAction(sub)
                        prev_dividend = None
                        continue

                if row["Typ operacji"] == "" and (
                    row["Opis"] == cleanText("Saldo początkowe") or row["Opis"] == cleanText("Saldo końcowe")
                ):
                    continue
                ...
                if row["Typ operacji"] == cleanText("Blokady pod zlecenia") or row[
                    "Typ operacji"
                ] == cleanText("Transakcje"):
