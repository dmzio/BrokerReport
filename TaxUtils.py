...
def getNBPValue(value, curency, time):
    ...
    while True:
        try:
            data = requests.get(
                "http://api.nbp.pl/api/exchangerates/rates/A/%s/%s?format=json" % (curency, ntime)
            )
...
def getECBCurrencyTable(curency):
    data = requests.get(
        "https://www.ecb.europa.eu/stats/policy_and_exchange_rates/euro_reference_exchange_rates/html/eurofxref-graph-%s.en.html"
        % (curency.lower())
    ).content.decode()
    table = {}
