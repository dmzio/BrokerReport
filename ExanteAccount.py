...
class ExanteAccount(Account):
    ...
    def _import(self, folder):
        ...
        for name in os.listdir(folder):
            ...
            for row in reversed(csv):
                ...
                main = Action(
                    ...
                    self.stock(
                        None if row[C_ISIN] == "None" else row[C_ISIN],
                        row[C_SYMBOL].split(".", 1)[0],
                        e,
                        row[C_CURRENCY],
                    )
                    if row[C_TYPE] != "FOREX"
                    else self.currency(row[C_SYMBOL].split("/")[0]),
                )
                self._add(main)

                v = (
                    Decimal(fixNumber(row[C_VOLUME])) * (Decimal(-1) if k else Decimal(1))
                    if not row[C_SYMBOL].endswith(".E.FX")
                    else Decimal(fixNumber(row[C_GAIN]))
                )
                if not v.is_zero():
                    sub = Action(
                        time,
                ...
                if fee:
                    ...
                    transations.append(
                        (time, row[C_SYMBOL], "COMMISSION", fee * Decimal(-1), row[C_COMMISSION_CURRENCY])
                    )
                ...
                transations.append(
                    (
                        ...
                        "TRADE",
                        main.count,
                        str(main.asset) if not row[C_SYMBOL].endswith(".E.FX") else row[C_SYMBOL],
                    )
                )
        ...
        def eraseTransation(time, symbol, row, transations):
            found = [
                idx
                for idx, x in enumerate(transations)
                if time.date() == x[0].date()
                and symbol == x[1]
                and row[F_TYPE] == x[2]
                and Decimal(fixNumber(row[F_SUM])) == x[3]
                and row[F_ASSET] == x[4]
            ]
            if not found:
                return False
        ...
        while autoconversions:
            ...
            if (
                autoconversions
                and autoconversions[0][2][F_SYMBOL] == row[F_SYMBOL]
                and autoconversions[0][2][F_WHEN] == row[F_WHEN]
            ):
                ...
                main = Action(
                    time, EActionType.PAYMENT, Decimal(fixNumber(minus[F_SUM])), self.currency(minus[F_ASSET])
                )

                sub = Action(
                    time, EActionType.BUY, Decimal(fixNumber(plus[F_SUM])), self.currency(plus[F_ASSET])
                )
                sub.addAction(main)
                self._add(sub)
                continue
        ...
        while financial:
            ...
            if row[F_TYPE] == "DIVIDEND" and " tax " in row[F_COMMENT]:
                ...
                main = Action(
                    ...
                    self.stock(
                        ...
                        currency=row[F_ASSET],
                    ),
                )

                income = Action(
                    time, EActionType.INCOME, Decimal(fixNumber(row[F_SUM])), self.currency(row[F_ASSET])
                )
            ...
            if (
                row[F_TYPE] == "EXERCISE"
                and financial
                and financial[0][2][F_TYPE] == row[F_TYPE]
                and financial[0][2][F_COMMENT] == row[F_COMMENT]
                and financial[0][2][F_SYMBOL] == row[F_SYMBOL]
                and row[F_COMMENT].endswith(" Exercised Rights")
            ):
            ...
            if (
                row[F_TYPE] == "CORPORATE ACTION"
                and financial
                and financial[0][2][F_TYPE] == "CORPORATE ACTION"
                and financial[0][2][F_WHEN] == row[F_WHEN]
                and financial[0][2][F_COMMENT] == row[F_COMMENT]
                and row[F_COMMENT] == "%s -> %s" % (financial[0][2][F_SYMBOL], symbol)
            ):
                self._assets.changeName(
                    financial[0][2][F_SYMBOL].split(".", 1)[0],
            ...
            if (
                row[F_TYPE] == "STOCK SPLIT"
                and "Stock Split " in row[F_COMMENT]
                and len(financial) > 1
                and financial[0][2][F_TYPE] == "STOCK SPLIT"
                and financial[0][2][F_SYMBOL] == row[F_SYMBOL]
                and financial[1][2][F_TYPE] == "STOCK SPLIT"
                and financial[1][2][F_SYMBOL] == row[F_SYMBOL]
            ):
                row_cash = row if "Fractional Share Payment" in row[F_COMMENT] else None
                if not row_cash:
                    row_cash = (
                        financial[0][2]
                        if "Fractional Share Payment" in financial[0][2][F_COMMENT]
                        else financial[1][2]
                    )
                ...
                if not row_old:
                    row_old = (
                        financial[0][2]
                        if financial[0][2][F_ASSET] == row[F_SYMBOL] and int(financial[0][2][F_SUM]) < 0
                        else financial[1][2]
                    )
                ...
                main = Action(getTime(row_old[F_WHEN]), EActionType.SELL, sold, asset)
                income = Action(
                    getTime(row_cash[F_WHEN]),
                    EActionType.INCOME,
            ...
            if (
                ...
                and financial[0][2][F_TYPE] == "STOCK SPLIT"
                and financial[0][2][F_COMMENT] == row[F_COMMENT]
                and financial[0][2][F_ASSET] == row[F_ASSET]
            ):
