...
class ExcelOutput:
    ...
    def __addTaxSummary(self, worksheet):
        ...
        value_names = [
            FIELD_EQUITY_PIT8C_COST,
            FIELD_EQUITY_PIT8C_INCOME,
            FIELD_EQUITY_OTHER_COST,
            FIELD_EQUITY_OTHER_INCOME,
            FIELD_EQUITY_SUM,
            FIELD_DIVIDEND_LOCAL,
            FIELD_DIVIDEND_REMOTE_PAYED,
            FIELD_DIVIDEND_REMOTE_TAX,
            FIELD_DIVIDEND_REMOTE_SUM,
        ]
        ...
        total_format = self._excel.add_format(
            {"bold": True, "num_format": "#,##0.00", "font_color": "white", "bg_color": "#4F81BD"}
        )
        ...
        for year in reversed(years):
            ...
            if [x for x, y in self._taxCountryCalculations.keys() if x == year]:
                ...
                worksheet.add_table(
                    ...
                    {
                        "columns": [
                            {"header": "Account", "header_format": center_format},
    ...
    def __addAccountHistoryPage(self, account):
        years = list(
            reversed(
                sorted(list(set([x.time.year for x in account.actions if x.type != EActionType.DIVIDEND])))
            )
        )
        if not years:
            return False
        ...
        for action in reversed(account.actions):
            if action.type == EActionType.DIVIDEND:
                continue
            row = self.__visitAction(
                years,
                worksheet,
                row,
                0,
                action,
                None if account.taxType == ETaxType.NO_TAX else calcTax,
                sum_tax,
                Rounding.TAX_TRANSACTION_COST_ROUNDING,
                Rounding.TAX_TRANSACTION_GAIN_ROUNDING,
                account.taxType == ETaxType.MANUAL,
            )
        ...
        worksheet.add_table(
            ...
            {
                "columns": [
                    {"header": "Date", "header_format": center_format},
                    {"header": "Ticker"},
                    {"header": "Name"},
                    {"header": "Country"},
        ...
        columns = [
            {
                "header": FIELD_EQUITY_PIT8C_COST
                if account.taxType == ETaxType.PIT8C
                else FIELD_EQUITY_OTHER_COST,
                "header_format": center_format,
            },
            {
                "header": FIELD_EQUITY_PIT8C_INCOME
                if account.taxType == ETaxType.PIT8C
                else FIELD_EQUITY_OTHER_INCOME,
                "header_format": center_format,
            },
    ...
    def __addAccountDividendPage(self, account):
        ...
        for action in reversed(account.actions):
            ...
            row = self.__visitAction(
                ...
                0,
                action,
                None if account.taxType == ETaxType.NO_TAX else localCalcTax,
                sum_tax,
                Rounding.TAX_TRANSACTION_DIVIDEND_TAX_PAYED_ROUNDING,
                Rounding.TAX_TRANSACTION_DIVIDEND_TAX_BASE_ROUNDING,
            )
        ...
        for year in years:
            ...
            sum_tax[year][0] = Rounding.round(
                sum_tax[year][0], Rounding.TAX_ACCOUNT_DIVIDEND_TAX_BASE_ROUNDING
            )
            sum_tax[year][1] = Rounding.round(
                sum_tax[year][1], Rounding.TAX_ACCOUNT_DIVIDEND_TAX_PAYED_ROUNDING
            )
            sum_tax[year][2] = Rounding.round(
                sum_tax[year][2], Rounding.TAX_ACCOUNT_DIVIDEND_TAX_BASE_ROUNDING
            )
    ...
    def __visitAction(
        ...
        roundCost=None,
        roundIncome=None,
        calcCountryTax=False,
    ):
        nrow = row
        color = "black" if level == 0 else "#333333"
        formats = [
            self._excel.add_format({"font_color": color, "bold": level == 0, "num_format": "yyyy-mm-dd"}),
        ...
        worksheet.write_string(
            nrow, 2, action.asset.name if action.asset.name else action.asset.ticker, formats[2]
        )
        ...
        if funcColumns:
            italic = self._excel.add_format(
                {"font_color": color, "bold": level == 0, "num_format": "#,##0.00"}
            )
            non_italic = self._excel.add_format(
                {"font_color": color, "bold": level == 0, "num_format": "#,##0.00"}
            )
            italic.set_italic()
            ...
            for year, value in flat_tax.items():
                ...
                if 0 == level:
                    ftax = funcColumns(
                        (Rounding.round(value[0], roundCost), Rounding.round(value[1], roundIncome))
                    )
                ...
                for idx, it in enumerate(ftax):
                    if not it.is_zero():
                        worksheet.write_number(
                            nrow, begin + idx, it, italic if it != ttax[idx] else non_italic
                        )
    ...
    def __visitAsset(self, worksheet, row, asset, count, time):
        formats = [
            self._excel.add_format({"font_color": "black", "bold": False, "num_format": "yyyy-mm-dd"}),
    ...
    def __addAccountAssetsPage(self, account):
        ...
        worksheet.add_table(
            ...
            {
                "columns": [
                    {"header": "Last Change", "header_format": center_format},
                    {"header": "Ticker"},
                    {"header": "Name"},
